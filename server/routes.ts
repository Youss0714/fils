...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/accounting/chart-of-accounts", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      res.status(500).json({ message: "Erreur lors de la récupération du plan comptable" });
    }
  });

  app.get("/api/accounting/chart-of-accounts/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const id = parseInt(req.params.id);
      const account = await storage.getChartOfAccount(id, userId);
      if (!account) {
  ...
  app.post("/api/accounting/chart-of-accounts", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const accountData = insertChartOfAccountsSchema.parse({ ...req.body, userId });
  ...
  app.patch("/api/accounting/chart-of-accounts/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const id = parseInt(req.params.id);
      const updates = req.body;
      const account = await storage.updateChartOfAccount(id, updates, userId);
      res.json(account);
    } catch (error) {
      console.error("Error updating chart of account:", error);
      res.status(400).json({ message: "Erreur lors de la mise à jour du compte" });
    }
  });

  app.delete("/api/accounting/chart-of-accounts/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const id = parseInt(req.params.id);
      await storage.deleteChartOfAccount(id, userId);
  ...
  app.get("/api/accounting/trial-balance", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const trialBalance = await storage.getTrialBalance(userId, periodStart as string, periodEnd as string);
      res.json(trialBalance);
    } catch (error) {
      console.error("Error fetching trial balance:", error);
      res.status(500).json({ message: "Erreur lors de la récupération de la balance de vérification" });
    }
  });

  app.post("/api/accounting/trial-balance/generate", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      ...
      const trialBalance = await storage.generateTrialBalance(userId, periodStart, periodEnd);
      res.json(trialBalance);
    } catch (error) {
      console.error("Error generating trial balance:", error);
      res.status(500).json({ message: "Erreur lors de la génération de la balance de vérification" });
    }
  });
